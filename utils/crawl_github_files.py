...
if __name__ == "__main__":
    ...
    import json
    with open('github_files_result.json', 'w', encoding='utf-8') as f:
        # Convert sets to lists before JSON serialization
        result_copy = {
            "files": result["files"],
            "stats": {
                **result["stats"],
                "include_patterns": list(result["stats"]["include_patterns"]) if result["stats"]["include_patterns"] else None,
                "exclude_patterns": list(result["stats"]["exclude_patterns"]) if result["stats"]["exclude_patterns"] else None
            }
        }
